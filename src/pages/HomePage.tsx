...
export function HomePage() {
  ...
  const startExam = async (type: ExamType) => {
    ...
    if (!key) {
      alert("Lütfen önce Ayarlar'dan AI API Anahtarınızı girin.");
      navigate('/settings');
      return;
    }
  ...
  const historyTitle = useMemo(() => (history.length ? `Geçmiş Sonuçlar (${history.length})` : 'Geçmiş Sonuçlar'), [history.length]);
    ...
    <div className="space-y-6">
      ...
      {!apiKey && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-1">Soru üretebilmek için Ayarlar'dan Gemini API anahtarını girmelisin.</p>
      ...
      <div className="mt-8 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {history.length > 0 && (
            <Button variant="ghost" size="sm" onClick={() => { storage.clearHistory(); setHistory([]); }}>
              Temizle
            </Button>
          )}
        </div>
        ...
        {history.length === 0 ? (
          <div className="text-center py-8 text-gray-400 bg-white rounded-xl border border-dashed border-gray-200">Henüz sınav çözmediniz.</div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500 flex items-center justify-end gap-1 mt-1">
                      <Clock size={14} /> {Math.round(h.durationMs / 60000)} dk
                    </div>
                  </div>
                </div>
